...
**分数按时间加权，且要自己收手才算数。** 每秒 10、击落一颗 5、护盾拦下一颗 2：后来的陨石更快更密，
活着本身就是分数；纯击落数会奖励「只打不挡」，拦截给分是让护盾的加压有回报。温度到 640K（熔融刚起、
`hot3` 已播）走既有的引力挤压——**得分归零**：行星先你一步死去，你什么也没保住。自己按下武器
（握拳 / 摊掌）结束这一局，**+20**（四颗的分），分才算入账。于是「什么时候收手」成了这个模式真正的决策：拖得越久分越高，也越可能被下一颗砸穿。
...
一局结束时把观测者本人画出来：**魔**（你亲手压碎了它）、**人**（行星先你一步死去，0 分）、**神**
（得分 ≥ 250——五十颗的分，只有自己收手才到得了）。画像由 Gemini 按当时的表情生成：16:9 横幅高清（1K 约 1344×768，`GEMINI_IMAGE_SIZE=2K` 更锐），
一律明亮、脸清晰且经过修饰，背景按档位各是一个世界（神：天光云海与光翼；魔：熔岩与火星；人：明亮的观测室）。
每档是一整套人物设定：人是持双枪的守卫者，魔手持熔岩黑曜石长矛，神背后是光环与光翼、掌中有星球；
