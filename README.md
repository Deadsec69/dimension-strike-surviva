...
任何确认：准星压到石头上就打（两发之间留 0.12s，扫过一片也不会一帧全清）。护盾仍要停住 0.45s——
它是要占地方的东西，不该被划过去的手顺手放下。迟滞带里发 `aim(null)`（✌️ 在 0.35s 松开不会被 120ms
的退出延迟送到 0.45s），停留遇到 80ms 以内的空档只暂停不清零。护盾要松开 ✌️ 才重新武装：一次比划一块。
...
**护盾的代价是气压。** 一块平面挡下撞击，但它是加进大气里的东西：气压走对数轴，两块之后宜居
...
打下来的才是；拦截给分是让护盾的加压有回报。难度也只看分：起步慢，125 分（二十五颗）快一点，
200 分（四十颗）再快，往后每 100 分再加一成——目标值按 τ=4s 逼近，换档读作「快了一点」而不是跳变。
温度到 640K（熔融刚起、`hot3` 已播）走既有的引力挤压——分照算，只是没有奖励；自己按下武器
（握拳 / 摊掌）结束这一局，**+20**（四颗的分）。560K 起画面正中挂预警（610K 转红），告诉你该收手了。
评级只看分：100 分半神、250 分神；不到 100 分：自己收手的是魔，烧穿的是人。判词在生存模式里说的是你——
...
一局结束时把观测者本人画出来：**魔**（不到 100 分就亲手压碎了它）、**人**（不到 100 分且行星先你一步死去）、
**半神**（≥100 分，二十颗）、**神**（≥250 分，五十颗）。画像由 Gemini 按当时的表情生成：16:9 横幅高清（1K 约 1344×768，`GEMINI_IMAGE_SIZE=2K` 更锐），
